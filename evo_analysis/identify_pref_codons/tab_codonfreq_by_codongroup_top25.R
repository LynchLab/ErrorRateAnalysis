# get codon freqs from highly-expressed genes and arrange in matrix form by codon groups
...
fout = 'output/codongroup_codonfreq_top25.csv'
...
dab <- read.csv(fab)
cat(sprintf("# genes = %i\n", nrow(dab)))
## only keep the highest abundance level
dab <- dab[ dab$level == 4, ]
cat(sprintf("# top 25 percent genes by abundance = %i\n", nrow(dab)))
